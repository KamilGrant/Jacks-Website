import { motion } from 'framer-motion';
import { SITE } from '../data/content';

/* Floating social sidebar — desktop only */
const SOCIALS = [
  {
    label: 'Facebook',
    href: SITE.facebook,
    color: '#1877F2',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
      </svg>
    ),
  },
  {
    label: 'Instagram',
    href: SITE.instagram,
    color: '#E1306C',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
        <circle cx="12" cy="12" r="4"/>
        <circle cx="17.5" cy="6.5" r="1.5" fill="currentColor" stroke="none"/>
      </svg>
    ),
  },
  {
    label: 'Google',
    href: SITE.google,
    color: '#EA4335',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
      </svg>
    ),
  },
];

export default function SocialSidebar() {
  return (
    <>
      <motion.div
        initial={{ x: -60 }}
        animate={{ x: 0 }}
        transition={{ delay: 1.5, duration: 0.5, ease: 'easeOut' }}
        style={s.sidebar}
        className="social-sidebar"
      >
        {SOCIALS.map((sc, i) => (
          <motion.a
            key={sc.label}
            href={sc.href}
            target="_blank"
            rel="noreferrer"
            aria-label={sc.label}
            style={s.item}
            initial={{ x: -40, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ delay: 1.6 + i * 0.1 }}
            whileHover={{ x: 8, backgroundColor: sc.color, color: 'white' }}
          >
            <span style={s.icon}>{sc.icon}</span>
            <span style={s.label}>{sc.label}</span>
          </motion.a>
        ))}
      </motion.div>

      <style>{`
        .social-sidebar { display: flex !important; }
        @media (max-width: 1024px) { .social-sidebar { display: none !important; } }
      `}</style>
    </>
  );
}

const s: Record<string, React.CSSProperties> = {
  sidebar: {
    position: 'fixed', left: 0, top: '50%', transform: 'translateY(-50%)',
    zIndex: 900, display: 'flex', flexDirection: 'column', gap: 2,
  },
  item: {
    display: 'flex', alignItems: 'center', gap: 10,
    padding: '12px 16px', background: 'white',
    color: 'var(--clr-mid)', borderRadius: '0 10px 10px 0',
    boxShadow: '2px 2px 12px rgba(0,0,0,.1)',
    transition: 'all .25s', fontWeight: 600, fontSize: '.78rem',
    overflow: 'hidden', maxWidth: 44,
  },
  icon:  { flexShrink: 0, display: 'flex', alignItems: 'center' },
  label: { whiteSpace: 'nowrap' as const, overflow: 'hidden' },
};
